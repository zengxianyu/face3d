...
def estimate_shape3d(x, shapeMU, shapePC, shapeEV, expression, s, R, t3d, lamb = 3000):
    '''
    Args:
        x: (2, n). image points (to be fitted)
        shapeMU: (3n, 1)
    ...
    n = x.shape[1]
    sigma = shapeEV
    t3d = np.array(t3d)[..., None]
    A = s*R

    # --- calc pc
    pc_3d = np.resize(shapePC.T, [dof, n, 3]) # 199 x n x 3
    pc_3d = np.reshape(pc_3d, [dof*n, 3]) 
    pc_3d = pc_3d.dot(A.T.copy()) # 199 x n x 2
    
    pc = np.reshape(pc_3d, [dof, -1]).T # 2n x 199
    ...
    exp_3d = expression
    # 
    b = A.dot(mu_3d + exp_3d) + t3d # 3 x n
...
def estimate_expression3d(x, shapeMU, expPC, expEV, shape, s, R, t3d, lamb = 2000):
    '''
    Args:
        x: (3, n). image points (to be fitted)
        shapeMU: (3n, 1)
        expPC: (3n, n_ep)
        expEV: (n_ep, 1)
        shape: (3, n)
    ...
    pc_3d = np.reshape(pc_3d, [dof*n, 3]) 
    pc_3d = pc_3d.dot(A.T) 
    pc = np.reshape(pc_3d, [dof, -1]).T # 3n x 29
    ...
    shape_3d = shape
    # 
    b = A.dot(mu_3d + shape_3d) + t3d # 2 x n
    b = np.reshape(b.T, [-1, 1]) # 3n x 1
...
def fit_points(x, X_ind, model, n_sp, n_ep, max_iter = 4):
    ...
    for i in range(max_iter):
        ...
        if x.shape[0] == 2:
            ...
            P = mesh.transform.estimate_affine_matrix_3d23d(X.T, x.T) 
        else:
        ...
        if x.shape[0] == 2:
            ep = estimate_expression(x, shapeMU, expPC, model['expEV'][:n_ep,:], shape, s, R, t[:2], lamb = 2000) # 20
        elif x.shape[0] == 3:
            ep = estimate_expression3d(x, shapeMU, expPC, model['expEV'][:n_ep,:], shape, s, R, t, lamb = 2000)
        else:
            raise NotImplementedError
        ...
        if x.shape[0] == 2:
            sp = estimate_shape(x, shapeMU, shapePC, model['shapeEV'][:n_sp,:], expression, s, R, t[:2], lamb = 4000) # 40
        elif x.shape[0] == 3:
            sp = estimate_shape3d(x, shapeMU, shapePC, model['shapeEV'][:n_sp,:], expression, s, R, t, lamb = 4000)
        else:
            raise NotImplementedError
